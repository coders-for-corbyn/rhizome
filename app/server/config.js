...
class Config {
  ...
  static _loadSettings() {
    ...
    let local = settings.local[process.env.RHIZOME_SERVER_ID];
    if (!local) {
      throw new Error(`Missing local environment settings for RHIZOME_SERVER_ID: ${process.env.RHIZOME_SERVER_ID}`); // eslint-disable-line max-len
    }
